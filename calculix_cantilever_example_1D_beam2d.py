...
modelSpace= predefined_spaces.StructuralMechanics2D(preprocessor.getNodeHandler)
...
pt1= points.newPoint(geom.Pos3d(0.0,8.0,0.0))  # Right end
...
beamSection= crossSection.defElasticShearSection2d(preprocessor,EL)
...
#########################################################
# Boundary conditions.
modelSpace.fixNode000(pt0.getNode().tag)  # Fix all the 6 DOF of the node
                                              # at pt0.
...
lp0.newNodalLoad(pt1.getNode().tag,xc.Vector([9.0e6,0.0,0.0]))
...
oh.outputStyle.cameraParameters.viewUpVc= [1,0,0]
