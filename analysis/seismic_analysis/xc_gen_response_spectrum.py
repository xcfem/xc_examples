...
'''
This scipt calculates the maximum acceleration for a single-degree-of-freedom model with varying natural periods, forced into motion by the same dinamic event, that is given as parameter as an earthquake's time-history recording.

The file used in this example as time-history input is organized in the following columns:
...
recAccel.callbackRecord= "cAccel[self.tag].append([self.getDomain.getTimeTracker.getCurrentTime,self.getAccel])"


prep.getDomain.setTime(timeVals[0])
solProc= predefined_solutions.PenaltyNewmarkNewtonRaphson(feProblem, numSteps= len(timeVals), timeStep= timeStep)
if(solProc.solve()!=0):
...
for T in periods:
    ...
    nTag=caseDct[T]['n2Tag']
    resAcc=cAccel[nTag]
    nodeAbsAcc=[abs(res[-1][0]) for res in  resAcc]
    caseDct[T]['maxAccel']=max(nodeAbsAcc)
