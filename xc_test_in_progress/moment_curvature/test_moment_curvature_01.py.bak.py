...
def moment_curvature(modelSpace, section, axialLoad, maxK, numIncr):
    ...
    myRecorder= feProblem.getDomain.newRecorder("node_prop_recorder",None)
    myRecorder.setNodes(xc.ID([nod1.tag]))
    myRecorder.callbackRecord= "modelSpace.mi.append(-self.getReaction[2])"
    ...
    constantTS= lPatterns.newTimeSeries("constant_ts","ts")
    ...
    solProc= predefined_solutions.PlainNewtonRaphson(prb= feProblem)
    solProc.setup()
    ...
    linearTS= lPatterns.newTimeSeries("linear_ts","ts")
    ...
    solProc= predefined_solutions.SimpleNewtonRaphsonDisplacementControl(prb= feProblem, node= nod2, dof= 2, increment= dK, numSteps= numIncr)
    solProc.setup()
    analysis= solProc.analysis
...
plt.plot(modelSpace.ky, modelSpace.mi, "-b")
